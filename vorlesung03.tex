Sei $(\Omega,\mathfrak{B},P)$ der W-Raum unserer Wahl. P habe Dichte $f$, also eine Teilfunktion 
\[f:R\rightarrow \int_{- \infty}^{+ \infty}{f(t)dt)}=1\]
\[F(x)=P((-\infty,x])=\int_{- \infty}^{+ \infty}{f(t)dt)},\ x \in \mathbb{R}\]
...
\[
	\int_0^x{\lambda \cdot exp(-\lambda t)dt} = \left[- exp(-\lambda t)\right]_o^x = 1-exp(-\lambda x) % aufleitung eigtl. mit unter und über ...
\]
...
\section{Weitere Eigenschaften von Wahrscheinlichkeitsmaßen}

%% TODO Suche und ersetze R durch \Oemga in W-Ma0en
Sei $(\Omega,\mathfrak{A},P)$ W-Raum. Seien $A_1,A_2,\ldots \in \mathfrak{A}$. Dann gilt
\begin{equation}
...
\end{equation}
Wir können die Vereinigung auch als Summe von disjunkten Ereignissen auffassen und gelangen zu dieser Schreibweise in \eqref{eq:subsigmaadd}. Deshalb gilt auch \eqref{eq:subsigmadisj}
...
Speziell folgt hieraus die so genannte ``Sub-Additivität von P'':
\begin{equation}
...
\noindent Natürlich kann man das auch allgemeiner schreiben:\\
