...
% X_j ~ B(1,p) je unabhängig, E(X_j) = p, Var(X_j) = p(1-p)
...
\subsection{Beispiel}
$X_1,X_2,\ldots$ sind unabhängige Zufallsvariablen, je $\mathfrak{P}(\lambda)$ verteilt. $\lambda = E(X_j) = Var(X_j)$. % Poisson
Dann
...
\section{Grenzwertsätze für ausgewählte Verteilungen}
\subsection{Lokaler zentraler Grenzwertsatz für die Poisson-Verteilung}
Tatsächlich lässt sich zeigen bei $X_\lambda \sim \mathfrak{P}(\lambda)$:
\begin{equation}
	P(X_\lambda=k) \sim \frac{1}{\sqrt{2\cdot\pi\cdot\lambda}}\cdot\exp{\left(-\frac{1}{2}\cdot\frac{(k-\lambda)^2}{\lambda}\right)}
\end{equation}
für $\lambda \rightarrow n, |k-\lambda|= \text{const}\cdot\sqrt{\lambda}$ das heißt
\begin{equation}
	\sup_{\substack{k\in\mathbb{N}\\|k-\lamda|\leq\text{const}\cdot\sqrt{\lambda}}}{\left| \frac{P(X_k=k)}{\frac{1}{\sqrt{2\cdot\pi\cdot\lambda}}\cdot\exp{(-0.5\cdot\frac{(k-\lambda)^2}{\lambda})}}-1 \right|}\rightarrow 0
\end{equation}

\subsection{Lokaler zentraler Grenzwertsatz für die Binomial-Verteilung}
Analaog für $X_n \sim \mathfrak{P}(n,p)$
\begin{equation}
...
\end{equation}
\subsection{Beweis}
Sei $f:\mathbb{R}^d\rightarrow\mathbb{R}$ stetig und beschränkt. Dann
\[ E(f(h(X_n))) = E(\underbrace{f\cdot h}_{\text{stetig, beschränkt}}(X_n)) \longrightarrow E(f\cdot h(X)) = E(f(h(X_n))) \]

\subsection{Satz}
Seien $X, X_n, Y_n$ reelle Zufallsvarieblen, $c \in \mathbb{R}$ konstant. Es gelte $X_n\stackrel{v}{\rightarrow}X, Y_n\stackrel{P}{\rightarrow}c$ (d.h.: Für jedes $\epsilon > 0$ gilt $\lim_{n\rightarrow{\infty}}{P(|Y_n-c|>\epsilon)=0}$) . Dann 
\begin{equation}
...
\end{equation}
Allgemein
\(f:\mathbb{R} \rightarrow \mathbb{R} \cup \{+\infty,-\infty\}, f|_U\) reellwertig für eine Umgebung U von $\mu$.
Dann gilt
...
% Varianz, Unabhängiglkeit von ZVA
