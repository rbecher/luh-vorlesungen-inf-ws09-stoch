...
\begin{enumerate}
	\item $X \sim Suett-B(n,p)$\\
				Ohne Einschränkung $X=\sum_{j=1}^n{I_{A_j}}$\\
				$p = P(A_j),\ j=1,\ldots,n$\\
				\[E(X) = \sum_{j=1}^n{\underbrace{E(I_{A_j})}_{=P(A)=p}}=n \cdot p\]
	\item $X \sim Suett-H(a,r,n)$\\
				$X = \sum_{j=1}^n{I_{A_j}}$ \\
				$A_{j} \stackrel{^}{=}$ Beim $n$-fachen Ziehen ohne Zurücklegen je eine Kugel aus einer Urne mit $r$ roten und $s=a-r$ schwarzen Kugeln und bei der $j$-ten Ziehung wird eine rote Kugel gezogen.\\
				...
				Beim $n$-fachen Ziehen mit Zurücklegen je eine Kugel aus einer Urne mit  $r$ roten und $s=a-r$ schwarzen Kugeln ist Anzahl $X$ der gezogenen roten Kugeln $Suett-B(n,\frac{r}{a}$-verteilt, also
				\[ E(X)=n\cdot\frac{r}{a} \]
\end{enumerate}
...
Für $1 \leq s \leq r, s \in \mathbb{N}$ gilt dann wegen $|X|^s \leq |X|^r+1$, dass $E(|X|^s) < \infty$ ist. 
Sei $X$ reeelle Zufallsvariable mit $E(X^2) < \infty$. Dann ist $E(|X|) < \infty$.

Für $a\in\mathbb{R}$ ist $|X-a|^2 \leq X^2+2\cdot |a|\cdot |X|+a^2$, also $E(|X-a|^2) < \infty$. Es ist
\begin{equation*}
	\begin{split}
...
Sei $X$ reelle Zufallsvariable mit $E(X^2) < \infty$. Dann heißt
\begin{equation}
...
\begin{enumerate}
	\item $X \sim Suett-R(a,b)$
	\item $X \sim Suett-N(a,\sigma^2)$
	\item $X \sim Exp(\lambda)$. $X$ hat Dichte 
	...
	\begin{equation}
		\begin{split}
			Var(X) &= \lambda\cdot\int_{0}^{+ \infty}{x^2\cdot\exp{-\lambda\cdot x}\cdot dx} - \left( \frac{1}{\lambda}\right)^2 \\
						 &= \frac{1}{\lambda^2}
		\end{split}
	\end{equation}
\end{enumerate}
...
\end{equation}
Mittels vollständiger Induktion folgt für reelle Zufallsvariablen $X_1,\ldots,X_n$ mit $E(X_j^2) < \infty, j=1,\ldots,n$.
\begin{equation*}
	\begin{split}
...
Also insbesondere $Cov(I_A,I_B)=0$ falls $A,B$ unabhängige Ereignisse.
Ferner gilt für die Ereignisse $A_1,\ldots,A_n$:
\begin{equation*}
	\begin{split}
...
Sei $X \sim Suett-B(n,p)$. Dann wäre eigentlich
\[ Var(X) = \sum_{k=0}^{n}{(k-n\cdot p)^2\cdot\binom{n}{k}\cdot p^k\cdot(1-p)^^{n-k}} \]
Wir vereinfachen das im Folgenden: O.B.d.A. sei $X=\sum_{j=1}^{n}{I_{A_i}}$ mit \textbf{unabhängigen} Ereignissen $A_i, i=1,\ldots,n$.
Dann
\begin{equation}
	Var(X) = \sum_{j=1}^{n}{Var(I_{A_i})} = n\cdot p \cdot (1-p)
\end{equation}

\subsubsection{Hypergeometrischverteilt}

Sei $X \sim Suett-H(a,r,n)$. Dann wäre eigentlich
\[ Var(X) = \sum_{k=\max{(0,n+r-a)}}^{\min{(r,n)}}{(k-n\cdot\frac{r}{a})^2\cdot \frac{\binom{r}{k}\cdot\binom{a-r}{n-k} }{\binom{a}{n}}} \]
Wir vereinfachen das im Folgenden: O.B.d.A. fassen wir $X$ auf als die Anzahl der gezogenen roten Kugeln in $n$ Ziehungen bei $n$-fachem Ziehen ohne Zurücklegen je eine Kugel aus einer Urne mit $r$ roten und $s=a-r$ schwarzen Kugeln. Dann $X=\sum_{j=1}^n{I_{A_i}}$ und mit $P(A_i)=\frac{r}{a}$ sei
\begin{equation}
	\begin{split}
		Var(X) &= n\cdot \frac{r}{a}\cdot(1-\frac{r}{a}) + n\cdot(n-1)\cdot \left[ \frac{r\cdot(r-1)}{a\cdot(a-1)} - \left( \frac{r}{a} \right)^2 \right] \\
					 &= \underbrace{n\cdot \frac{r}{a}\cdot\left( 1-\frac{r}{a} \right)}_{\text{Varianz beim Ziehen mit Zurücklegen}}\cdot \frac{a-n}{n-1}
	\end{split}
\end{equation}
da
