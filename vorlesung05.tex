...
$X_1,\ldots,X_n$ sind unabhängig, wenn gilt
\[ P(X_1\in B_1,\ldots,X_n\in B_n) = P(X_1\in B_1)\cdots P(X_n\in B_n) \]
für jede Auswahl von Ereignissen $B_i \in \mathfrak{A}, i=1,\ldots,n$
...
Die $X_1,\ldots,X_n$ haben je eine diskrete Verteilung.
			  Dann sind 
			  \[ X_1,\ldots,X_n \text{ unabhängig } \iff P(X_1=x_1,\ldots,X_n=x_n) = P(X_1=x_1)\cdots P(X_n = x_n)\] 
			  für jede Auswahl von Elementen $x_i \in R_i,\ i=1,\ldots,n$.\\
...
Sei $X=(X_1,\ldots,X_n)$ mit den reellen Zufallsvariablen $X_1,\ldots,X_n$.
				Es habe $X$ eine Dichte $f$, das heißt
...
\end{equation}
so sind die Zufallsvariablen $X_1,\ldots,X_n$ unabhängig.

\subsection{Beispiel}
Ein Einzelexperiment, bei dem ein interessierendes Ereignis $A$ mit Wahrscheinlichkeit $p\in [0,1]$ eintritt, wird $n$-mal unter identischen Versuchsbedingungen ohne gegenseitige Beeinflussung (unabhängige Versuchswiederholungen) wiederholt.\\
...
Die $X_1,\ldots,X_n$ sind unabhängig, denn
\begin{equation}
	\begin{split}
		&P(X_1=\epsilon_1,\ldots,X_n=\epsilon_n) \\
	= &P(\omega=\{1,\ldots,n\}\in\Omega;\ X_1(\omega_1,\ldots,\omega_n)=\epsilon_1,\ldots,X_n(\omega_1,\ldots,\omega_n)=\epsilon_n) \\
...
Eine reelle (oder $\{0,1\}$-wertige) Zufallsvariable $X$ heißt Binomialverteilt mit den Parametern $n\in\mathbb{N}$ und $p\in[0,1]$, wenn gilt
\begin{equation}
...
Sei $(\Omega,\mathfrak{A},P)$ W-Raum, $A\in \mathfrak{A}$ Ereignis. Dann heißt $I_A:\Omega \rightarrow \{0,1\}$ \textbf{Indikator}-funktion (oder Indikatorvariable) von $A$, definiert durch
...
Seien $A_1,\ldots,A_n\in \mathfrak{A}$ unabhängige Ereignisse mit $p=P(A_i), i=1,\ldots,n.$.\\
Dann gilt $\underbrace{X_1}_{=I_{A_1}},\ldots,\underbrace{X_n}_{=I_{A_n}}$ unabhängig und $X_i \sim \mathfrak{B}(1,p)$. $X=X_1+\ldots+X_n \sim \mathfrak{B}(n,p)$.

\subsection{Beispiel Urnenmodell}

Eine Urne enthält $r$ rote und $s$ schwarze Kugeln. Uns können verschiedene Modi interessieren.

\subsubsection{Mit Zurücklegen}

Es wird $n$-mal mit Zurücklegen je eine Kugel gezogen.\\
...
Es wird $n$-mal ohne Zurücklegen je eine Kugel gezogen mit $n\leq r+s =: a$.\\
				Sei $X$ Zufallsvariable und bezeichne die Anzahl der gezogenen roten Kugeln in $n$ Ziehungen.\\
...
heißt hypergeometrisch verteilt mit den Parametern $a,r,n\in\mathbb{N}, r\leq a, n\leq a$ und der Schreibweise
\begin{equation}
	X \sim \mathfrak{H}(a,r,n).
\end{equation}

Das bekannteste Beispiel dürfte das Zahlenlotto ``6 aus 49'' sein.